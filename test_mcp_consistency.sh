...
# MCP Consistency Test Script
# This script automatically starts the server, monitors SSE, and tests MCP format consistency

set -e

echo "🚀 Starting MCP Consistency Test..."
...
# Send MCP streaming chat completion request
echo "📤 Sending MCP streaming chat completion request..."
curl -s -X POST \
     ...
     -d '{
       ...
       "params": {
         ...
         "arguments": {
           ...
           "_meta": {
             "progressToken": "test_consistent_format_123"
           }
         }
       }
...
if grep -q "test_consistent_format_123" /tmp/paragonic_test/sse.log; then
