...
local function test_low_load_scenario()
    ...
    -- Assertions
    assert_test(load_result.completed_requests == scenario.total_requests, 
        "All requests should complete in low load scenario")
    assert_test(#load_result.errors == 0, 
        "No errors should occur in low load scenario")
    assert_test(response_stats.avg < 1000, 
        "Average response time should be under 1 second in low load")
    assert_test(final_metrics.active_connections == 0, 
        "No active connections should remain after low load test")
    assert_test(final_metrics.usage_percentage <= 100, 
        "Connection pool usage should be within limits")
...
local function test_medium_load_scenario()
    ...
    assert_test(#load_result.errors < 5, 
        "Fewer than 5 errors should occur in medium load scenario")
    assert_test(response_stats.avg < 2000, 
        "Average response time should be under 2 seconds in medium load")
    assert_test(final_metrics.active_connections == 0, 
        "No active connections should remain after medium load test")
...
local function test_high_load_scenario()
    ...
    -- Assertions
    assert_test(load_result.completed_requests >= scenario.total_requests * 0.95, 
        "At least 95% of requests should complete in high load scenario")
    assert_test(#load_result.errors < 20, 
        "Fewer than 20 errors should occur in high load scenario")
    assert_test(response_stats.avg < 5000, 
        "Average response time should be under 5 seconds in high load")
    assert_test(final_metrics.active_connections == 0, 
        "No active connections should remain after high load test")
...
local function test_stress_load_scenario()
    ...
    assert_test(load_result.completed_requests >= scenario.total_requests * 0.90, 
        "At least 90% of requests should complete in stress load scenario")
    assert_test(#load_result.errors < 50, 
        "Fewer than 50 errors should occur in stress load scenario")
    assert_test(response_stats.avg < 10000, 
        "Average response time should be under 10 seconds in stress load")
    assert_test(final_metrics.active_connections == 0, 
        "No active connections should remain after stress load test")
...
local function test_memory_usage_under_load()
    ...
    -- Assertions
    assert_test(final_metrics.total_connections <= 20, 
        "Total connections should not exceed pool size")
    assert_test(final_metrics.active_connections == 0, 
        "No active connections should remain after test")
    assert_test(final_metrics.available_connections >= 0, 
        "Available connections should be non-negative")
...
local function test_performance_comparison()
    ...
    -- Assertions
    assert_test(with_pooling_stats.avg <= without_pooling_stats.avg * 1.5, 
        "Pooling should not significantly degrade performance")
    assert_test(with_pooling_result.completed_requests >= without_pooling_result.completed_requests * 0.9, 
...
local function test_error_recovery_under_load()
    ...
    -- Assertions
    assert_test(final_metrics.active_connections == 0, 
        "No active connections should remain after error recovery test")
    assert_test(final_metrics.available_connections >= 0, 
        "Should have available connections after error recovery")
    assert_test(load_result.completed_requests > 0, 
        "Should complete some requests even with errors")
...
local function print_performance_report()
    ...
    for scenario_name, metrics in pairs(load_test_state.performance_metrics) do
        ...
        test_log(string.format("  Success Rate: %.2f%%", 
            (metrics.completed_requests - metrics.errors) / metrics.completed_requests * 100))
        test_log(string.format("  Response Time (ms) - Avg: %.2f, Min: %.2f, Max: %.2f", 
            metrics.response_stats.avg, metrics.response_stats.min, metrics.response_stats.max))
...
local function run_all_load_tests()
    ...
    -- Print test summary
    test_log("=======================================")
    test_log(string.format("Load Test Summary: %d total, %d passed, %d failed", 
        load_test_state.test_count, load_test_state.passed_count, load_test_state.failed_count))
