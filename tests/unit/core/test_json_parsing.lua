...
local function test_json_parsing_decode_rpc_responses()
    ...
    local chat_response = '{"jsonrpc":"2.0","result":{"message":{"role":"assistant","content":"Hello! How can I help you today?"}},"id":1}'
    local parsed_chat = paragonic.parse_json_response(chat_response)
    ...
    local models_response = '{"jsonrpc":"2.0","result":{"models":[{"name":"llama2","size":"3.8GB"},{"name":"mistral","size":"4.1GB"}]},"id":1}'
    local parsed_models = paragonic.parse_json_response(models_response)
    ...
    local projects_response = '{"jsonrpc":"2.0","result":{"projects":[{"id":"1","name":"Test Project","description":"A test project"}]},"id":1}'
    local parsed_projects = paragonic.parse_json_response(projects_response)
...
local function test_json_parsing_error_handling()
    ...
    local parsed_error = paragonic.parse_json_response(invalid_json)
    ...
    local parsed_malformed = paragonic.parse_json_response(malformed_json)
