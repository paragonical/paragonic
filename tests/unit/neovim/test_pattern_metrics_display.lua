...
local function test_pattern_metrics_display()
    ...
    assert(math.abs(stats.success_rate - metrics.summary.success_rate) < 0.01, 
           "Success rate should match between stats and metrics")
    assert(math.abs(stats.average_execution_time_ms - metrics.summary.average_execution_time_ms) < 0.1, 
