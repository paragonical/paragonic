...
local function test_mcp_initialization_without_stream_id_errors()
	...
	if not session_success then
		assert_false(session_err:find("Invalid stream ID"), "Session initialization should not fail with 'Invalid stream ID'")
		assert_false(session_err:find("stream_id"), "Session initialization should not fail with stream_id related errors")
	end
end
