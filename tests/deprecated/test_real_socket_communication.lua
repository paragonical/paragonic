...
local function test_real_socket_communication()
	...
	local hello_curl = io.popen(
		'curl -s -X POST -H "Content-Type: application/json" -d \'{"jsonrpc":"2.0","method":"hello","params":{},"id":1}\' http://127.0.0.1:3000'
	)
	if hello_curl then
...
local function test_chat_completion_via_curl()
	...
	local chat_curl = io.popen(
		'curl -s -X POST -H "Content-Type: application/json" -d \'{"jsonrpc":"2.0","method":"chat_completion","params":["Hello, what is 2+2?","llama2"],"id":1}\' http://127.0.0.1:3000'
	)
	if chat_curl then
...
local function test_list_models_via_curl()
	...
	local models_curl = io.popen(
		'curl -s -X POST -H "Content-Type: application/json" -d \'{"jsonrpc":"2.0","method":"list_models","params":{},"id":1}\' http://127.0.0.1:3000'
	)
	if models_curl then
