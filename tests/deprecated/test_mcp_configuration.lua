...
local function test_mcp_configuration()
	...
	local vim_mock = {
		...
		fn = {
			stdpath = function(path_type)
				if path_type == "data" then
					return "/tmp/nvim-data"
				elseif path_type == "config" then
			...
			filereadable = function(file_path)
				if file_path:find("test_config.json") then
					return 1
				else
					return 0
				end
	...
	local validate_result =
		M.handle_configuration_method("config/validate", { key = "ollama_host", value = "http://localhost:11434" })
	assert(validate_result.valid == true, "Should validate correct value")

	local validate_invalid_result =
		M.handle_configuration_method("config/validate", { key = "log_level", value = "invalid" })
