...
local function test_mcp_additional_resources()
    ...
    local vim_mock = {
        ...
        fn = {
            getreg = function(reg)
                if reg == "a" then return "yanked text from register a"
                elseif reg == "b" then return "yanked text from register b"
                elseif reg == "c" then return "yanked text from register c"
                elseif reg == "0" then return "last yanked text"
                elseif reg == "1" then return "last deleted text"
                elseif reg == "\"" then return "default register content"
                else return "" end
            end,
            getregtype = function(reg)
                if reg == "a" then return "v"
                elseif reg == "b" then return "V"
                elseif reg == "c" then return "c"
                elseif reg == "0" then return "v"
                elseif reg == "1" then return "V"
                elseif reg == "\"" then return "v"
                else return "" end
            end,
            getchar = function() return 97 end, -- 'a'
            input = function(prompt) return "test macro" end,
            execute = function(cmd)
                if cmd == "let @a='dd'" then return 0
                elseif cmd == "let @b='yy'" then return 0
                else return 0 end
            end,
        ...
        json = {
            ...
            decode = function(json_str)
                ...
                elseif json_str:find("plugin") then
                    return {
                        {name = "nvim-treesitter", loaded = true, path = "/usr/share/nvim/runtime/plugin/treesitter.vim"},
                        {name = "telescope.nvim", loaded = true, path = "/home/user/.local/share/nvim/site/pack/packer/start/telescope.nvim/plugin/telescope.vim"}
                    }
                else
                    return {}
                end
