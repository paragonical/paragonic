...
local function test_mcp_progress_tracking()
	...
	local vim_mock = {
		api = {
			...
			nvim_buf_get_lines = function(buf, start, end_, strict)
				...
				elseif buf == 2 then
					return { "function test()", "  return true", "end" }
				else
					return {}
				end
			end,
	...
	assert(
		M.progress_state.active_operations[progress_id].name == "file_processing",
		"Should have correct operation name"
	)
	...
	function M.complete_progress_operation(progress_id, final_message)
		...
		local final_notification =
			M.create_progress_notification(progress_id, final_message or operation.message, 100, true)
	...
	function M.handle_tool_call_with_progress(id, params)
		...
		elseif tool_name == "agent_save_file" then
			...
			-- Start progress
			progress_id, start_notification =
				M.start_progress_operation("file_save", "Saving file: " .. (file_path or "current file"))
			table.insert(progress_notifications, start_notification)
	...
	function M.format_progress_summary(progress_notifications)
		local summary = {
			...
			end_percentage = progress_notifications[#progress_notifications]
					and progress_notifications[#progress_notifications].params.percentage
				or 0,
			final_message = progress_notifications[#progress_notifications]
					and progress_notifications[#progress_notifications].params.message
				or "",
			completed = progress_notifications[#progress_notifications]
					and progress_notifications[#progress_notifications].params.done
				or false,
		}
		return summary
	end
