...
local function test_get_server_info_consistency()
	...
	assert(
		info1.version == info2.version and info2.version == info3.version,
		"Server version should be consistent across calls"
	)
	assert(
		info1.status == info2.status and info2.status == info3.status,
		"Server status should be consistent across calls"
	)
...
local function test_get_server_info_with_server_restart()
	...
	print(
		"✓ Server info with server restarted: " .. info3.name .. " v" .. info3.version .. " (" .. info3.status .. ")"
	)
