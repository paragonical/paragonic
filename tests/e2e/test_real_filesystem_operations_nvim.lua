-- Test Real File System Operations in Neovim
-- This test should be run within Neovim using :source test_real_filesystem_operations_nvim.lua
...
-- Test 6: Test deferred function timing
local function test_deferred_function_timing()
    print("=== Test 6: Deferred Function Timing ===")
    
    print("  📝 Testing deferred function timing...")
    ...
    vim.defer_fn(function()
        ...
        print("    ⏰ Deferred function called after: " .. string.format("%.2f", delay) .. "ms")
    end, 100)
    
    -- Wait a bit to see if deferred function is called
    vim.wait(200, function()
        return deferred_called
    end)
    
    if deferred_called then
        print("    ✅ Deferred function timing is working correctly")
        return true
    else
        print("    ❌ Deferred function timing issue detected")
        return false
    end
end

-- Main test execution
print("=== Real File System Operations Test in Neovim ===")
...
if passed == total then
    ...
    print("• File system, memory, or timing issues detected")
