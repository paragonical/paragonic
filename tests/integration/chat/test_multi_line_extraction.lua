...
local function test_new_commands_available()
	...
	assert(
		type(paragonic.chat.send_message_backward_only) == "function",
		"send_message_backward_only should be available"
	)
	assert(
		type(paragonic.chat.send_message_forward_only) == "function",
		"send_message_forward_only should be available"
	)
	assert(type(paragonic.chat.send_message_command) == "function", "send_message_command should be available")
...
local function test_extraction_functions_exposed()
	...
	assert(
		type(paragonic.chat._test_extract_backward_to_tombstone) == "function",
		"_test_extract_backward_to_tombstone should be exposed"
	)
	assert(
		type(paragonic.chat._test_extract_forward_to_tombstone) == "function",
		"_test_extract_forward_to_tombstone should be exposed"
	)
	assert(
		type(paragonic.chat._test_extract_complete_range) == "function",
...
local function test_backward_compatibility()
	...
	assert(type(paragonic.chat.send_message_command) == "function", "Original send_message_command should still work")
	assert(type(paragonic.chat.open_chat) == "function", "open_chat should still work")
