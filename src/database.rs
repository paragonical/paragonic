...
pub async fn initialize() -> ParagonicResult<()> {
    ...
    DB_POOL
        .set(Arc::new(pool))
        .map_err(|_| ParagonicError::Internal("Database pool already initialized".to_string()))?;
...
pub async fn initialize_for_testing() -> ParagonicResult<()> {
    ...
    match initialize_embedded_db(&config).await {
        ...
        Err(e) => {
            warn!(
                "Failed to initialize test database: {}. Tests will run without database.",
                e
            );
...
pub fn get_pool() -> ParagonicResult<Arc<Pool<ConnectionManager<PgConnection>>>> {
    ...
    if cfg!(test) && std::env::var("USE_MOCK_DATABASE").is_ok() {
        return Err(ParagonicError::Internal(
            "Mock database mode enabled - no real database available".to_string(),
        ));
    }
...
pub fn get_database_config_from_manager(
    config_manager: &crate::config::ConfigManager,
) -> DatabaseConfig {
...
mod tests {
    ...
    fn test_get_pool_uninitialized() {
        ...
        match result {
            Err(ParagonicError::Internal(msg)) => {
                ...
                assert!(
                    msg.contains("Database not initialized")
                        || msg.contains("Mock database mode enabled")
                );
            }
            ...
            Err(ParagonicError::Internal(msg)) => {
                ...
                assert!(
                    msg.contains("Database not initialized")
                        ...
                        || msg.contains("Database not initialized for tests")
                        || msg.contains("Mock database mode enabled")
                );
            }
            _ => panic!("Expected Internal error"),
    ...
    async fn test_embeddings_migration() {
        ...
        assert!(
            table_exists.exists,
            "Embeddings table should exist after migration"
        );
