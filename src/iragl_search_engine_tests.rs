use crate::iragl::{IraglSearchRequest, IraglSearchResponse, IraglSearchResult, ContentAssociationResponse};
...
mod iragl_search_engine_tests {
    ...
    async fn test_organizational_context_weighting() {
        ...
        for context in contexts {
            ...
            let result = engine.apply_organizational_context_weighting(&request).await;
            assert!(result.is_ok());
...
impl IRAGLSearchEngine {
    ...
    pub fn average_response_time_ms(&self) -> u64 {
        ...
        self.total_response_time.load(std::sync::atomic::Ordering::Relaxed) / count
    }
    ...
    pub async fn perform_vector_similarity_search(&self, request: &IraglSearchRequest) -> ParagonicResult<IraglSearchResponse> {
        ...
        self.total_response_time.fetch_add(response_time, std::sync::atomic::Ordering::Relaxed);
    ...
    pub async fn apply_organizational_context_weighting(&self, request: &IraglSearchRequest) -> ParagonicResult<IraglSearchRequest> {
        // Mock implementation - just return the request as-is
        Ok(request.clone())
    }
    
    pub async fn rank_and_score_results(&self, mut results: Vec<IraglSearchResult>) -> ParagonicResult<Vec<IraglSearchResult>> {
        // Sort by similarity score (highest first)
        results.sort_by(|a, b| b.similarity_score.partial_cmp(&a.similarity_score).unwrap_or(std::cmp::Ordering::Equal));
        Ok(results)
    }
    
    pub async fn optimize_query(&self, request: &IraglSearchRequest) -> ParagonicResult<IraglSearchRequest> {
        // Mock implementation - just return the request as-is
        Ok(request.clone())
    }
    
    pub async fn apply_filters(&self, request: &IraglSearchRequest, _filters: &[String]) -> ParagonicResult<IraglSearchRequest> {
        // Mock implementation - just return the request as-is
        Ok(request.clone())
    }
    ...
    pub fn reset_statistics(&self) {
        ...
        self.total_response_time.store(0, std::sync::atomic::Ordering::Relaxed);
