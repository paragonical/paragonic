...
mod knowledge_stream_processor_tests {
    ...
    async fn test_knowledge_stream_processor_creation() {
        ...
        assert!(
            processor.is_ok(),
            "KnowledgeStreamProcessor should be created successfully"
        );
    ...
    async fn test_knowledge_stream_processor_with_config() {
        ...
        assert!(
            processor.is_ok(),
            "KnowledgeStreamProcessor should be created with config"
        );
    ...
    async fn test_automatic_content_association() {
        ...
        assert!(
            result.is_ok(),
            "Content processing with auto-association should succeed"
        );
        ...
        tracing::info!(
            "Association count: {} (may be 0 if database not initialized)",
            association_count
        );
    }
    ...
    async fn test_automatic_association_creation() {
        ...
        let request = IngestKnowledgeStreamRequest {
            content_type: "communication".to_string(),
            content_text: "This is test content that should trigger automatic associations"
                .to_string(),
            source_entity_type: "project".to_string(),
            source_entity_id: Uuid::new_v4(),
        ...
        tracing::info!(
            "Created {} automatic associations (may be 0 if database not initialized)",
            association_count
        );
    }
    ...
    async fn test_automatic_association_with_different_entity_types() {
        ...
        for entity_type in entity_types {
            ...
            assert!(
                response.is_ok(),
                "Auto-association should work with entity type: {}",
                entity_type
            );
    ...
    async fn test_automatic_association_disabled() {
        ...
        assert!(
            response.is_ok(),
            "Content processing should succeed even with auto-association disabled"
        );
        ...
        assert_eq!(
            association_count, 0,
            "No associations should be created when auto-association is disabled"
        );
    }
    ...
    async fn test_processor_statistics() {
        ...
        assert_eq!(processor.error_count(), 1);
        assert!(
            (processor.success_rate() - 0.5).abs() < 0.01,
            "Success rate should be approximately 0.5"
    ...
    async fn test_embedding_generation_integration() {
        ...
        assert!(
            response.is_ok(),
            "Content processing should succeed and generate embeddings"
        );
    ...
    async fn test_embedding_generation_with_different_models() {
        ...
        for model in models {
            ...
            assert!(
                response.is_ok(),
                "Embedding generation should work with model: {}",
                model
            );
    ...
    async fn test_embedding_generation_with_ollama_client() {
        ...
        let result =
            crate::iragl::generate_real_embeddings_for_knowledge_streams("nomic-embed-text").await;
        ...
        match result {
            Ok(count) => {
                tracing::info!(
                    "Successfully generated embeddings for {} knowledge streams",
                    count
                );
            ...
            Err(e) => {
                tracing::info!(
                    "Ollama client test failed (expected if Ollama not running): {}",
                    e
                );
    ...
    async fn test_embedding_generation_with_mock_data() {
        ...
        let result =
            crate::iragl::generate_embeddings_for_knowledge_streams("nomic-embed-text").await;

        // Print the actual result for debugging
        match &result {
            Ok(count) => {
                tracing::info!(
                    "Mock embedding generation succeeded: {} knowledge streams",
                    count
                );
            }
            Err(e) => {
        ...
        match result {
            ...
            Err(e) => {
                if e.to_string().contains("Database not initialized") {
                    ...
                    assert!(
                        true,
                        "Database not initialized is expected in test environment"
                    );
                } else {
                    assert!(
                        false,
                        "Mock embedding generation failed with unexpected error: {:?}",
                        e
                    );
                }
            }
    ...
    async fn test_error_counting_with_validation() {
        ...
        assert_eq!(
            ...
            1,
            "Validation errors should only count as 1 error"
        );
        assert_eq!(
            processor.processed_count(),
            0,
            "Invalid content should not be counted as processed"
        );
    }
}
...
impl KnowledgeStreamProcessor {
    ...
    pub async fn validate_content(
        ...
        if request.content_text.trim().is_empty() {
            return Err(ParagonicError::InvalidInput(
                "Content text cannot be empty".to_string(),
            ));
        }
    ...
    pub async fn process_content(
            ...
            match self.process_content_with_retry(&request, attempt).await {
                    ...
                    if self.config.enable_auto_association {
                        match self.create_automatic_associations(&response).await {
                            Ok(association_count) => {
                                self.last_association_count
                                    .store(association_count, std::sync::atomic::Ordering::Release);
                            }
                            Err(e) => {
                                tracing::warn!(
                                    "Failed to create automatic associations (non-blocking): {}",
                                    e
                                );
                                self.last_association_count
                ...
                Err(e) => {
                    ...
                    if attempt < self.config.max_retries {
                        ...
                        tracing::warn!(
                            "Processing attempt {} failed, retrying in {}ms: {}",
                            attempt + 1,
                            delay,
        ...
        self.error_count
            .fetch_add(1, std::sync::atomic::Ordering::AcqRel);
        Err(last_error
            .unwrap_or_else(|| ParagonicError::Internal("Unknown error occurred".to_string())))
    }

    /// Process content with specific retry attempt
    async fn process_content_with_retry(
        &self,
        request: &IngestKnowledgeStreamRequest,
        attempt: usize,
    ) -> ParagonicResult<KnowledgeStreamResponse> {
    ...
    async fn create_automatic_associations(
        &self,
        response: &KnowledgeStreamResponse,
    ) -> ParagonicResult<usize> {
        let mut association_count = 0;

        println!(
            "DEBUG: Starting automatic association creation for content {}",
            response.id
        );
        ...
        tracing::info!(
            "Created {} automatic associations for content {}",
            association_count,
            response.id
        );
        Ok(association_count)
    ...
    pub async fn process_batch(
        &self,
        requests: Vec<IngestKnowledgeStreamRequest>,
    ) -> ParagonicResult<Vec<KnowledgeStreamResponse>> {
        ...
        tracing::info!(
            "Batch processing completed: {} successful, {} errors",
            responses.len(),
            errors
        );
