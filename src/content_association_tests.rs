...
mod content_association_engine_tests {
    use super::*;
    use crate::iragl::{ContentAssociationResponse, CreateContentAssociationRequest};
...
impl ContentAssociationEngine {
    ...
    pub async fn calculate_association_strength(
        &self,
    ...
    pub async fn validate_entity_relationship(
            ...
            || request.association_strength > self.config.max_association_strength
        {
            return Err(ParagonicError::InvalidInput(
                "Association strength out of bounds".to_string(),
            ));
        }
        ...
        {
            return Err(ParagonicError::InvalidInput(
                "Confidence score out of bounds".to_string(),
            ));
        }
    ...
    pub async fn classify_association_type(
        &self,
        request: &CreateContentAssociationRequest,
    ...
    pub async fn create_association(
        ...
        if self.config.enable_duplicate_prevention {
            if self.is_duplicate_association(&request).await? {
                return Err(ParagonicError::InvalidInput(
                    "Duplicate association detected".to_string(),
                ));
            }
        }
    ...
    async fn is_duplicate_association(
        &self,
        request: &CreateContentAssociationRequest,
