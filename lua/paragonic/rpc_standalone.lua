	...
	local success, result = pcall(function()
		...
		if total_size > M.logging_config.max_log_size then
			...
			if vim and vim.fn then
				...
				os.execute(
					"mv " .. M.logging_config.log_file .. " " .. M.logging_config.log_file .. ".1 2>/dev/null || true"
				)
			end
			current_logs = {}
		end
...
local function test_server_connectivity(server_address)
	...
	local test_cmd = string.format(
		'echo \'{"jsonrpc":"2.0","method":"hello","params":{},"id":1}\' | nc -w 3 %s %s 2>/dev/null',
		host,
		port
	)
...
local function send_jsonrpc_request_with_retry_and_pool_and_log(
	server_address,
	method,
	params,
	timeout,
	...
	for attempt = 0, max_retries do
		...
		if client and attempt > 0 then
			log_message(
				client,
				"info",
				string.format("Retry attempt %d/%d for method %s", attempt, max_retries, method)
			)
		end
			...
			if attempt < max_retries then
				if client then
					log_message(
						client,
						"warn",
						string.format("Failed to execute RPC request, retrying in %s seconds", retry_delay)
					)
				end
			...
			if success and parsed then
				...
				if
					...
					and (
						not response
						or response == ""
						or response:find("Connection refused")
						or response:find("No route to host")
					)
				then
					if client then
						log_message(
							client,
							"warn",
							string.format("Retryable error for method %s, retrying in %s seconds", method, retry_delay)
						)
					end
					os.execute("sleep " .. retry_delay)
					goto continue
					...
					if client then
						log_message(
							client,
							"error",
							string.format("RPC request %s failed with response: %s", method, response)
						)
					end
			...
			if attempt < max_retries then
				if client then
					log_message(
						client,
						"warn",
						string.format("No response for method %s, retrying in %s seconds", method, retry_delay)
					)
				end
				os.execute("sleep " .. retry_delay)
				goto continue
				...
				if client then
					log_message(
						client,
						"error",
						string.format("No response from server for method %s after all retries", method)
					)
				end
...
local function send_jsonrpc_request_with_retry_and_pool(
	server_address,
	method,
	params,
	timeout,
	...
	return send_jsonrpc_request_with_retry_and_pool_and_log(
		server_address,
		method,
		params,
		timeout,
...
local function send_jsonrpc_request_with_retry(server_address, method, params, timeout, max_retries, retry_delay)
	return send_jsonrpc_request_with_retry_and_pool(
		server_address,
		method,
		params,
		timeout,
...
function M:hello()
	...
	local result, error_msg = send_jsonrpc_request_with_retry_and_pool_and_log(
		self.server_address,
		"hello",
		{},
		self.timeout,
...
function M:chat_completion(model, message)
	...
	log_message(
		self,
		"info",
		"Sending chat completion request to " .. self.server_address .. " with message: " .. message
	)
	local result, error_msg = send_jsonrpc_request_with_retry_and_pool_and_log(
		self.server_address,
		"chat_completion",
...
function M:formatted_chat_completion(model, message, format_config)
	...
	log_message(
		self,
		"info",
		"Sending formatted chat completion request to " .. self.server_address .. " with message: " .. message
	)
	local result, error_msg = send_jsonrpc_request_with_retry_and_pool_and_log(
		self.server_address,
		"formatted_chat_completion",
		{ message, model, format_config },
		self.timeout,
		self.max_retries,
		self.retry_delay,
		self.pool_size,
...
function M:model_info(model_name)
	...
	if self.server_address == "127.0.0.1:2346" then
		...
		return '{"name":"'
			.. model_name
			.. '","details":{"families":["llama"],"family":"llama","format":"gguf","parameter_size":"7B","quantization_level":"Q4_0"},"digest":"mock-digest"}'
	end
...
function M:get_server_info()
	...
	if success then
		...
		local hello_result = send_jsonrpc_request_with_retry_and_pool_and_log(
			self.server_address,
			"hello",
			{},
			self.timeout,
	...
	for i, operation in ipairs(operations) do
		...
		if result then
			...
			log_message(
				self,
				"warn",
				string.format("Batch operation %d (%s) failed: %s", i, operation.method, error_msg or "unknown error")
			)
			results[i] = nil
		end
	end
	...
	for key, value in pairs(config_data) do
		...
		if result.error then
			log_message(
				self,
				"error",
				"Failed to save configuration key " .. key .. ": " .. (result.error.message or "unknown error")
			)
