...
function mcp_performance.check_performance_alerts(metrics)
	...
	for _, alert in ipairs(alerts) do
		...
		-- Log alert
		print(string.format("[PERFORMANCE %s] %s: %s", alert.level, alert.category, alert.message))
	end
end
...
function mcp_performance.record_request(start_time, end_time, success)
	...
	if success then
		...
		performance_state.success_rate = (
			(performance_state.request_count - performance_state.error_count) / performance_state.request_count
		) * 100
	end
